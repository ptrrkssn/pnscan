     ...
     char *argv[])
{
  ...
  EndOptions:
    ...
    {
	if (i + 1 == argc)
	    if (get_service(argv[i], &first_port) != 1)
	    {
		fprintf(stderr, "%s: Invalid port/service: %s\n",
			argv[0], argv[i]);
		exit(1);
	    }
	...
	{
	    if (!isatty(0))
		fprintf(stderr, "[Reading host/address data from stdin]\n");
	    else
		fprintf(stderr, "[Waiting for host/address data on stdin]\n");
	}
